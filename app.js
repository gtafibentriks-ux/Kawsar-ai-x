...
async function generateImage() {
  ...
  try {
    const res  = await fetch("/api/imagine", {
      method: "POST",
      headers: { "Content-Type":"application/json" },
      body: JSON.stringify({ prompt, style: imgStyle, width: imgWidth, height: imgHeight, mode: "generate" })
    });
    const data = await res.json();
...
async function editImage() {
  ...
  if (!editBase)  { alert("Please upload an image to edit first!"); return; }
  ...
  try {
    const res  = await fetch("/api/imagine", {
      ...
      headers: { "Content-Type":"application/json" },
      body: JSON.stringify({
        prompt: `Edit this image: ${prompt}. Original image context: make the edit while preserving the style.`,
        style: imgStyle, width: imgWidth, height: imgHeight, mode: "edit"
      })
    });
    const data = await res.json();
...
async function generateVariation() {
  if (!varBase) { alert("Please upload an image first!"); return; }
  const prompt = document.getElementById("var-prompt").value.trim() || "Create a variation";
  setGenLoading(true, "Creating variation…");
  try {
    const res  = await fetch("/api/imagine", {
      method: "POST",
      headers: { "Content-Type":"application/json" },
      body: JSON.stringify({ prompt: `Create a variation: ${prompt}`, style: imgStyle, width: imgWidth, height: imgHeight, mode: "variation" })
    });
    const data = await res.json();
    ...
    setGenLoading(false);
    alert("Variation failed: " + err.message);
  }
}
